...
def showcase():
    """Some code to showcase the syntax - source MagicPython.
    https://github.com/MagicStack/MagicPython/blob/master/misc/example.py
    ...
    def get_regex(): return re.compile(     # type: ignore
        r"""\A
            word 
            (<|%3C)\s*?script[\s\S]*?(>|%3E)[\s\S]*?(<|%3C)\s*?(\/|%2F)\s*?script[\s\S]*?(>|%3E)/gims
            (?:                         # a comment
               (?P<fill>.)?
